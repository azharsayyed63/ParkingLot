...
function processUserCommands (input) {
    ...
    switch (userCommand) {
        ...
        case 'park':
            try {
                parkingSlotNumber = parkingLot.parkVehicle(input);
        ...
        case 'leave':
            try {
                parkingSlotNumber = parkingLot.leaveVehicle(input);
        ...
        case 'status':
            ...
            break;
        case 'registration_numbers_for_vehicles_with_type':
            var registrationNumbers = parkingLot.getVehiclesWithSameType(input);
            if (registrationNumbers) {
                console.log(registrationNumbers);
			}
			else {
                console.log(chalk.red('Sorry, Vehicle with given type is not found'));
            }
            break;
        case 'slot_numbers_for_vehicles_with_type':
            parkingSlotNumbers = parkingLot.getSlotsWithSameTypeVehicle(input);
            if (parkingSlotNumbers) {
                console.log(parkingSlotNumbers);
            }
            else {
                console.log(chalk.red.bold('Sorry, Vehicle with given type is not found'));
            }
            break;
        case 'slot_number_for_registration_number':
            parkingSlotNumber = parkingLot.getSlotByVehNumber(input);
            if (parkingSlotNumber) {
                console.log(parkingSlotNumber);
			}
			else {
                console.log(chalk.red.bold('Sorry, Vehicle with given registration number is not found'));
            }
            break;
        case 'leave_vehicle_by_registration_number':
            try {
                parkingSlotNumber = parkingLot.leaveVehicleByVehNumber(input);
                console.log(chalk.blue('Slot number ' + parkingSlotNumber + ' is free.'));
            }
            catch (err) {
                console.log(chalk.red('Sorry, vehicle with given registration is not found'));
            }
            break;
